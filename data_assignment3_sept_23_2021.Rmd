...
marriage_data <- read_csv("C:/Users/owenb/OneDrive/Documents/Harvard Sem 5/Gov 1372/gov_1372/Sep23ClassData_clean.csv")
```
...
marriage_data_new <- marriage_data %>% 
  mutate(strongPARTISAN = ifelse(strongPARTISAN == TRUE, "Strong Partisan", "Weak Partisan")) %>% 
  mutate(outPartyUnhappy = ifelse(outPartyUnhappy == TRUE, "Unhappy", "Happy")) %>% 
  mutate(inPartyHappy = ifelse(inPartyHappy == TRUE, "Happy", "Unhappy"))


graph_1 <- ggplot(data = marriage_data_new %>% filter(is.na(outPartyUnhappy)==FALSE)) +
  geom_bar(mapping = aes(x = outPartyUnhappy, y = ..prop.., group = 1), fill = "azure3", stat = "count") +
  facet_wrap(~strongPARTISAN) +
  ylab("Proportion") +
  xlab("Satisfaction with Out-Party Marriage") +
  labs(title = "Happiness with Out-Party Marriage by Partisanship Level") +
  theme_clean()

graph_2 <- ggplot(data = marriage_data_new %>% filter(is.na(inPartyHappy)==FALSE)) +
  geom_bar(mapping = aes(x = inPartyHappy, y = ..prop.., group = 1),  fill = "azure3", stat = "count") +
  facet_wrap(~strongPARTISAN) +
  ylab("Proportion") +
  xlab("Satisfaction with In-Party Marriage") +
  labs(title = "Happiness with In-Party Marriage by Partisanship Level") +
  theme_clean()
