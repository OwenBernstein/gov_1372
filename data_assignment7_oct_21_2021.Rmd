...
protest_df_bydate <- protest_df %>%  mutate(after = ifelse(day<as.Date("2020-05-25"), 0,1))
```
...
mod_1 <- lm(group_favorability_the_police ~ pid7 + after, data = protest_df_bydate)
summary(mod_1)
...
function_mod_dat <- protest_df_bydate %>% 
  mutate(black = ifelse(race_ethnicity == 2, 1, 0)) %>% 
  mutate(over_50 = ifelse(age > 50, 1, 0))

mod_2 <- lm(group_favorability_the_police ~ pid7 + after + black + female + over_50*pid7, data = function_mod_dat)
...
polr(data = function_mod_dat, formula = as.factor(group_favorability_the_police) ~ pid7 + after + black + female + over_50*pid7, method = "probit")
```
