...
overall_mod <- lm(Overall ~ BFI_extraversion + BFI_agreeableness + BFI_conscientiousness + BFI_emot_stability + BFI_openness, data = personality_data)

social_mod <- lm(Social ~ BFI_extraversion + BFI_agreeableness + BFI_conscientiousness + BFI_emot_stability + BFI_openness, data = personality_data)

economic_mod <- lm(Economic ~ BFI_extraversion + BFI_agreeableness + BFI_conscientiousness + BFI_emot_stability + BFI_openness, data = personality_data)

stargazer(overall_mod, social_mod, economic_mod, type = "text")
```

The above chart shows the results from three separate linear regressions. The first of these regressions uses the trait indices from the Big Five Inventory as the explanatory variables and self reported ideology (on a 1-5 scale with 1 being extremely liberal and 5 being extremely conservative) as the outcome variable. This regression shows that none of the trait indices (extraversion, agreeableness, conscientiousness, emotional stability, and openness) are statistically significant predictors of overall ideology. similarly, when using social and economic ideology as the outcome variable none of the trait indices are statistically significant predictors. 
